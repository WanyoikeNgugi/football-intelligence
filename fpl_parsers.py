...
def save_best_players(events, output_dir):
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(players_dir, "best_players.csv"), index=False
    )


def save_player_history(data, player_name, player_id, output_dir):
    player_history_dir = os.path.join(
        output_dir, "players", f"{player_name}_{player_id}"
    )
...
def save_manager_leagues(data, entry_id, output_dir):
    ...
    classic_leagues_df.to_csv(
        os.path.join(manager_dir, "classic_leagues.csv"), index=False
    )
...
def save_cleaned_players(output_dir):
    ...
    players_df["value_per_m"] = players_df["total_points"] / (
    ...
    players_df.to_csv(
        os.path.join(output_dir, "players", "cleaned_players.csv"), index=False
    )
