...
def recalculateTotalPoints(playerID, newPos):
    newPoints = 0
    oldPoints = 0
    available_gws = [int(f.replace('gw_', '').replace('.csv', '')) 
...
if __name__ == "__main__":
    ...
    print(f"Van Dijk (DEF to MID): {recalculateTotalPoints(playerID=373, newPos='MID')}")
    print(f"O'Reilly (DEF to MID): {recalculateTotalPoints(playerID=411, newPos='MID')}")
