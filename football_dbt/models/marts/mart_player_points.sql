...
aggregated as (
    select
        ...
        avg(case when was_home then total_points else null end) as avg_home_points,
        avg(case when not was_home then total_points else null end) as avg_away_points
    from gw
    group by
        player_id,
        player_name,
